...
def compose_analysis_prompt(user: str, company: dict, snap: dict | None = None, nws: dict | None = None, peers: dict | None = None, cycle: dict | None = None) -> str:
    ...
    return (
        ...
        "分析哲学（务必贯彻，源自《非共识的20分钟》打法）：公开信息大家都有，市场多数时候已把共识 price in。你的价值不在复述「这公司好不好」，而在**基于真实数据找出『现实可能与共识不同』的非共识点**，并诚实标注**你可能错在哪、概率多大**。重机制、轻点位——**不预测，拆传导**。\n\n"
        "报告结构（全部建立在【真实数据卡】上、引用具体数字）：\n\n"
        "## 〇、先问对问题\n"
        "市场现在主要在争论/定价这家公司的什么？这个问题**问对了吗**？如果问错了，真正该问的是什么？（很多错价源于大家盯错了变量。）\n\n"
        "## 一、市场共识：它在赌什么（真账 vs 赌注）\n"
        "用『估值共识·四工具』+ 反向DCF隐含增长说清当前价**隐含了什么预期**。然后把它拆成两栏：**①真账**＝数据已兑现、确凿支撑当前定价的部分；**②赌注**＝价格隐含、但尚未兑现、仍在赌的假设。你怀疑的应是②。\n\n"
        "## 二、第一性原理（生意的本质）\n"
        "把生意拆到不能再拆：本质卖什么价值、客户为什么非买不可、凭什么定价赚钱；护城河第一性来源是否成立/弱化。用毛利率/净利率/ROE + 同行质量分位佐证——**重点指出哪里与共识不一致**。\n\n"
        "## 三、资金传导（写成显式传导链）\n"
        "1) **内部链**：营收→毛利→营业利润→净利→自由现金流，画出这条链、点出**最脆的一环**（应收/存货/费用/含金量）；赚的钱去哪（再投资ROIC/分红/回购）。2) **资本结构/谁先拿钱**：负债端求偿排序与**必须现金支付的固定义务**（利息+分红）对现金流的覆盖是否紧张——会不会被迫靠外部融资续命。3) **宏观传导**：利率/流动性/政策经哪几环传导到它（用宏观资金传导信号）。4) 产业链上下游议价权与利润池。**每一处标出与共识假设矛盾的地方**。\n\n"
        "## 四、历史镜像（放进历史看）\n"
        "用历史估值分位 + 盈利周期位置判断当前在自身历史的位置；**市场是否把当前的峰值/谷值当常态外推**？给一个明确的历史类比框定：**「这次更像历史上的 ___ 还是 ___？」**，并说明像/不像在哪。\n\n"
        "## 五、非共识判断 + 反证（最关键）\n"
        "综合上文，给出 1–3 条**非共识论点**（数据显示 X，而共识/价格隐含 Y，差距多大、依托哪个支柱）。每条必须配：\n"
        "- **赔率与盈亏不对称**：最坏亏多少 / 最好赚多少 / 各自约概率——是不是「赔率≥2:1」「不对称」的位置（避免压路机前捡硬币）。\n"
        "- **我可能错在哪 + 概率**：列出最可能推翻该论点的理由，给一个校准过的概率（如『60% 成立 / 40% 只是暂时性资本开支周期』），参照基率；若判断里有你的情绪/立场投射，主动标注「需打折」。\n"
        "- **拥挤与反身性**：这笔共识交易是否**拥挤**？有没有「溢价飞轮 / 强制平仓 / 安全资产误认」式的**自我强化结构**，一旦反转会放大？\n"
